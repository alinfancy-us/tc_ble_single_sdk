...
/*
 * 中文说明：
 * 本文件是 BLE Remote（BLE 遥控器）工程的功能裁剪与参数配置头文件，集中管理各功能模块的使能开关
 * （低功耗管理、Flash 保护、OTA、安全配对、音频、红外发射/学习、按键扫描等）、调试打印开关、
 * 按键映射表以及系统时钟等参数配置。B85/B87/TC321X 芯片差异部分通过 BOARD_SELECT 等宏区分，
 * 具体功能实现请参见对应 .c 源文件。
 */
#pragma once
