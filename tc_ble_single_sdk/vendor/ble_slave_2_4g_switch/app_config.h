...
/*
 * 中文说明：
 * 本文件为 ble_slave_2_4g_switch 工程的应用层配置文件，集中定义了功能开关（电源管理、
 * 安全配对、OTA、Flash保护、电量检测等）、2.4G私有协议测试模式选择、调试日志开关、
 * 板级选型（区分B85/B87/TC321X）、UI（键盘/LED/按键）配置以及深度睡眠标志位等宏定义。
 * 其中按板级区分的分支（B87、TC321X专属部分）仅对相应芯片生效，不在本次改动范围内。
 */
